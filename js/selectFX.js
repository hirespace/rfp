...
(function (window) {
	...
	SelectFx.prototype._init = function () {
		...
		var selectedOpt = this.el.querySelector('option[selected]'),
			selectedOptValue = $(selectedOpt).text();
		this.hasDefaultPlaceholder = selectedOpt && selectedOpt.disabled;

		// get selected option (either the first option with attr selected or just the first option)
		this.selectedOpt = selectedOpt || this.el.querySelector('option');
		this.label = selectedOpt ? selectedOptValue : $(this.el).data('label');
	...
	SelectFx.prototype._createSelectEl = function () {
		...
		var selected = $(this.el).find("option[selected]"),
			selectedValue = selected.length > 0 ? selected.text() : false;

		var opts_el = '<div class="cs-options"><ul>' + options + '</ul></div>';
		this.selEl = document.createElement('div');
		this.selEl.className = this.el.className + (selectedValue ? " cs-select-filled" : "");
		this.selEl.tabIndex = this.el.tabIndex;
		this.selEl.innerHTML = '<span class="cs-label"><span class="cs-label-content"><span class="' + this.iconset + ' ' + this.iconset + '-' + this.icon + '" style="position: relative;"></span>' + this.label + '</span></span><span class="cs-placeholder"><span class="cs-placeholder-content">' + (selectedValue ? selectedValue : "") + '</span></span>' + opts_el;
