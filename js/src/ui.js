(function () {
	...
	$("#vProgress li").click(function () {
		section.set($(this).attr("toggle-section"));
	});
	...
	function stepSet(step) {
		var activeStepToBe = _.indexOf(stepsKeys, step),
			valid = checkForm();

		// @TODO Bad, bad nesting.. Sorry about that
		if (activeStepToBe > activeStep) {
			if (!valid) {
				return false;
			}
		}

		activeStep = activeStepToBe;
		assignActive();
	...
	function assignActive() {
		...
		vProgress.find(".glyphicon").addClass("hide");
		...
		_.forEach(children, function (ele) {
			...
			if (toggleSection == section.active()) {
				...
				var offset = toggleSection,
					index = _.indexOf(stepsKeys, offset);

				if (index < activeStep) {
					$(ele).find(".glyphicon").removeClass("hide");
				}
